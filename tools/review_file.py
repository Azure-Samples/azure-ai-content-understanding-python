...
def run_llm_review(file_path: str, file_content: str) -> str:
    ...
    prompt = (
        ...
        f"4. Edit the text directly to enhance readability and technical accuracy.\n"
        f"5. Preserve the original meaning and intent of the content.\n"
        f"6. Ensure consistency in terminology, tone, and technical details across README files, code comments, and markdown sections.\n\n"
        f"Output:\n"
...
def run_llm_comment_on_patch(patch: str) -> str:
    ...
    prompt = (
        ...
        f"```\n{patch}\n```\n\n"
        f"The changes were made by a previous editor to improve the code or documentation.\n"
        f"Your tasks:\n"
        f"1. Identify the categories of changes made. Use one or more of the following labels: **typo**, **grammar**, **clarity**, or **consistency**.\n"
        f"2. For each significant change, provide a summary with the following format:\n"
        f"  - **change**: [Brief description of the modification]\n"
        f"    - **rationale**: [Why this change was made]\n"
        f"    - **impact**: [How it improves the documentation or code]\n\n"
