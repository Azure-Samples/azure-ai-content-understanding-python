...
The folders [document_training](../data/document_training/) and [field_extraction_pro_mode](../data/field_extraction_pro_mode) contain manually labeled data used for training in Standard mode and reference documents in Pro mode as quick samples. Before using these knowledge source files, you will need an Azure Storage blob container to store them. Please follow the steps below to prepare your data environment:

1. **Create an Azure Storage Account:**  
   If you don’t already have one, please follow the guide to [create an Azure Storage Account](https://aka.ms/create-a-storage-account).  
   > If you already have an account, you may skip this step.

2. **Install Azure Storage Explorer:**  
   Azure Storage Explorer is a tool that simplifies working with Azure Storage data. Please install it and log in with your credentials by following the [installation guide](https://aka.ms/download-and-install-Azure-Storage-Explorer).
...
4. **Set SAS URL-related Environment Variables in the `.env` File:**  
   Depending on the sample you plan to run, configure the required environment variables in the [.env](../notebooks/.env) file. You have two options to set up environment variables that utilize the necessary Shared Access Signature (SAS) URL.
...
5. **Set Folder Prefixes in the `.env` File:**  
   Depending on the sample you will run, please set the required environment variables in the [.env](../notebooks/.env) file.
...
Once these steps are completed, your data environment will be ready. You may proceed to create an analyzer through code.